...
1. CLI flags
  1. Examples: `--project-excludes <value>`, `--python-version <value>`
2. Environment variable overrides -- this is the same as `PYREFLY_<config option name>`
  1. Examples: `export PROJECT_EXCLUDES=<value>`, `PYTHON_VERSION=<value> pyrefly ...`
3. Configuration options
  1. Examples: (in a `pyrefly.toml`) `project-excludes = <value>`, `python-version = <value>`
4. Pyrefly defaults
  1. See [Default `pyrefly.toml`](#default-pyreflytoml) for the default values used
...
1. Use [`python-interpreter`](#python-interpreter) if it's set by a flag or config option.
2. Find a `venv` at the root of the project by searching for something that looks like a
   Python interpreter, and looking for a `pyvenv.cfg` file in known locations.
3. Query `$(which python3)` and `$(which python)` (platform independent) to use
