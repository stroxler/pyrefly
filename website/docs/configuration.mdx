...
search paths that are automatically added, and are affected by this flag.
...
1. Adding your import root to the end of your search path. Your import root is
   a `src/` directory in the same directory as a config file *or* the
   config file's directory itself if no `src/` directory can be found.
