...
found and imported from (including modules in [`project_includes`](#project-includes)). This takes
...
- `ignore_errors_in_generated_code`: whether to ignore type errors in generated code. If enabled, generated files
  ...
  - Equivalent configs: none
- `use_untyped_imports`: whether to search imports in `site_package_path` that do not have a `py.typed` unconditionally.
  - Type: bool
  - Default: false
  - Flag equivalent: none
  ...
  - Notes: if a `<library>-stubs` package exists in `site_package_path`, we will search that first. If an import can't be
    found, we only fall back to a regular `<library>` package if no relevant `-stubs` package exists, or any package contains a
    `py.typed` file with `partial\n` in accordance with
    [the typing spec](https://typing.python.org/en/latest/spec/distributing.html#partial-stub-packages). For all non-`-stubs`
    packages, a `py.typed` file is required to use the types, unless a `-stubs` package specifies partial, or
    `use_untyped_imports` is true.
