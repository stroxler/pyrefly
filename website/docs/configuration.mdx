...
1. Adding your import root to the end of your search path. Your import root is
   ...
   *or* the config file's directory itself if none of the previously mentioned directories
...
**`conda-environment`, `python-interpreter-command`, `conda-environment`,
...
`which <your command>`, and automatically fill in `python-interpreter` for you with
the found path. Pyrefly will automatically search for `python3` and `python` on your
path when this option and [`python-interpreter`](#python-interpreter) are unset if
...
- Flag equivalent: `--python-interpreter-command`
- Notes:
  - This executes the value present in the `python-interpreter-command` field without
    any checks. It could be a security risk if your `python-interpreter-command` is an
...
- Equivalent configs:
  ...
  in mypy
- Notes:
    - Setting `<error-code> = true` is the same as having no error code configuration present, which means
    ...
    - If you want to disable type errors in IDE mode, you can also set
      ...
      automatically disable *all* type errors and Pyrefly diagnostics in the IDE.
...
*only* want to see type errors on CLI/CI runs.
