...
export class State {
    ...
    gotoDefinition(line, column) {
        const ret = wasm.state_gotoDefinition(this.__wbg_ptr, line, column);
        return ret;
    }
    /**
     * @param {number} line
     ...
     */
    inlayHint() {
        const ret = wasm.state_inlayHint(this.__wbg_ptr);
        return ret;
    }
}
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('pyrefly_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
