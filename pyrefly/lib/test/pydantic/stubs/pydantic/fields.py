...
from typing import Annotated, Any, cast, ClassVar, final, Literal, overload, TypeVar
from warnings import warn

import annotated_types
import typing_extensions
from pydantic_core import MISSING, PydanticUndefined
from typing_extensions import deprecated, Self, TypeAlias, Unpack
...
class FieldInfo(_repr.Representation):
    ...
    def __init__(self, **kwargs: Unpack[_FieldInfoInputs]) -> None:
        ...
        kwargs = {
            k: _DefaultValues.get(k) if v is _Unset else v for k, v in kwargs.items()
        }  # type: ignore
    ...
    def _construct(cls, metadata: list[Any], **attr_overrides: Any) -> Self:
        ...
        for meta in metadata:
            if isinstance(meta, FieldInfo):
                ...
                if (
                    current_js_extra is not None
                    and "json_schema_extra" in merged_kwargs
                ):
                    ...
                    elif callable(existing_js_extra) and isinstance(
                        current_js_extra, dict
                    ):
                        warn(
                            "Composing `dict` and `callable` type `json_schema_extra` is not supported. "
                            "The `callable` type is being ignored. "
    ...
    def merge_field_infos(*field_infos: FieldInfo, **overrides: Any) -> FieldInfo:  # noqa
...
class ComputedFieldInfo:
    ...
    def _update_from_config(self, config_wrapper: ConfigWrapper, name: str) -> None:
        ...
        title_generator = (
            self.field_title_generator or config_wrapper.field_title_generator
        )
